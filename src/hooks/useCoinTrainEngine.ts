...
export const useCoinTrainEngine = () => {
  ...
  const connectWebSocket = useCallback(() => {
    ...
    const wsUrl = `wss://${projectId}.supabase.co/functions/v1/coin-train-engine`;
    ...
    ws.onmessage = (event) => {
      try {
        ...
        switch (message.type) {
          ...
          case 'bet_error':
          case 'cashout_error':
            // These are handled by callbacks in placeBet/cashout
            break;
        }
      } catch (e) {
    ...
    ws.onerror = (error) => {
      console.error('Coin Train WebSocket error:', error);
    };
      ...
      const handleMessage = (event: MessageEvent) => {
        try {
          ...
          if (message.type === 'bet_placed' && message.data.requestId === requestId) {
            ...
            console.error('Bet error:', message.data.error);
            wsRef.current?.removeEventListener('message', handleMessage);
          }
        } catch (e) {
          console.error('Error handling bet response:', e);
        }
      };
    ...
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      ...
      const handleMessage = (event: MessageEvent) => {
        try {
          ...
          console.error('Error handling cashout response:', e);
        }
      };
