...
export function CycleRacePredictions({ cyclistCustomization }: CycleRacePredictionsProps) {
  const queryClient = useQueryClient();

  const { data: predictions, isLoading } = useQuery({
    queryKey: ["cycling-race-predictions"],
    queryFn: async () => {
      ...
      return data as CyclingRacePrediction[];
    },
    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          A prediction is created automatically for every queued race - it is never a standalone
          value. Real players only ever see "House Prediction" (generated independently of the
          real outcome). Admins and marketers see both columns here for oversight; demo and bot
          accounts are served the real outcome by design.
        </p>
      </div>

      {/* House logic settings */}
      <Card>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Real Outcome</TableHead>
                <TableHead>House Prediction (shown to players)</TableHead>
            ...
            <TableBody>
              {predictions.map((prediction) => (
                <TableRow key={prediction.id}>
                  ...
                  <TableCell>{prediction.confidence_percentage}%</TableCell>
                  <TableCell>
